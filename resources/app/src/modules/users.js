...
export class UsersManager {
  ...
  /**
   * Устанавливает FriendsManager для проверки статуса друзей
   * @param {FriendsManager} friendsManager - Менеджер друзей
   */
  ...
  updateUsersList(users, onVolumeChange, myId = null) {
    ...
    // Очищаем контейнер и добавляем все карточки одной операцией
    this.usersEl.innerHTML = "";
    this.usersEl.appendChild(fragment);
  }
  ...
  createUserCard(id, data, onVolumeChange, myId = null) {
    ...
    const contextmenuHandler = (e) => {
      ...
      if (userContextMenu) {
        ...
        // Скрываем кнопку "Добавить в друзья", если это сам пользователь или уже друг
        ...
        if (addFriendBtn) {
          ...
          } else if (this.friendsManager && firebaseUserId && this.friendsManager.isFriend(firebaseUserId)) {
            console.log('🔵 Скрываем кнопку "Добавить в друзья" (уже друг)');
            addFriendBtn.style.display = 'none';
          } else {
