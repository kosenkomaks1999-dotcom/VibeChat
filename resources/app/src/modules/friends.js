...
export class FriendsManager {
  constructor(db, authManager, onNotificationSound, uiManager = null, onJoinRoom = null) {
    ...
    this.friendsCache = {}; // Кэш списка друзей для быстрой проверки
   ...
   * @param {string} userId - Firebase ID пользователя
   * @returns {boolean} true если пользователь друг
   */
  isFriend(userId) {
    return this.friendsCache && this.friendsCache[userId] && this.friendsCache[userId].status === 'accepted';
  }
  ...
  async renderFriends(friends) {
    ...
    try {
      // Сохраняем друзей в кэш для быстрой проверки
      ...
      this.friendsListEl.appendChild(fragment);
    ...
    this.db.ref(requestsPath).once('value').then((snap) => {
      ...
      this.requestsListener = (snap) => {
        ...
        if (disappearedRequests.size > 0) {
          ...
          // Проверяем, какие из исчезнувших запросов были действительно приняты (стали друзьями)
          ...
          Promise.all(
            acceptedFriendIds.map(async (friendId) => {
              try {
                ...
                const friendSnapshot = await this.db.ref(`users/${currentUser.uid}/friends/${friendId}`).once('value');
                ...
                console.error('Ошибка при проверке друга:', error);
                return null;
              }
            })
          ).then((friendNames) => {
            // Фильтруем null значения (те, кто не стал другом)
            const acceptedFriends = friendNames.filter(name => name !== null);
            
            // Показываем тост об успешном принятии заявки только если действительно стали друзьями
            if (this.uiManager && this.uiManager.showToast && acceptedFriends.length > 0) {
              if (acceptedFriends.length === 1) {
                this.uiManager.showToast(`${acceptedFriends[0]} принял(а) ваш запрос в друзья!`, 3000, 'success');
              } else {
                this.uiManager.showToast(`${acceptedFriends.length} пользователей приняли ваши запросы в друзья!`, 3000, 'success');
              }
            }
